libname ash '/home/cbb21460/test';
...
data test1;
set test;
trainperiod = TRAINMONTH;
IF TRAINMONTH="" THEN trainperiod = "None";
RUN;
...
infile '/home/cbb21460/test/aa.csv' delimiter = ',' MISSOVER DSD lrecl=32767 firstobs=2 ;
 input
  ID
  time
  nobs;
if _ERROR_ then call symputx('_EFIERR_',1);  /* set ERROR detection macro variable */
/*if nobs < 20 then nobs_cat = 2;
  else if nobs >= 20 then nobs_cat = 1;
    else nobs_cat = .;*/
run;

data test2;
set test1;
if trainperiod = "October" then new_train = 1;
  else if trainperiod = "March" then new_train = 2;
    else if trainperiod = "None" then new_train = 3;
      else new_train = .;

if level_educ = "A1" then new_educ = 2;
  else if level_educ = "A2" then new_educ = 1;
    else new_educ = .;
    
if expgr = '0-4' then new_expgr = 2;
  else if expgr = '>=4' then new_expgr = 1;
    else new_expgr = .;
RUN;
...
proc univariate noprint data=test3;
var nobs;
output out=quintile pctlpts=33 66 pctlpre=pct;
run;

/* write the cutpoints to macro variables */
data _null_;
set quintile;
call symput('q1',pct33) ;
call symput('q2',pct66) ;
run;

/* create a new variable containing the quintiles */
data test3;
set test3;
if nobs =. then x_quint = .;
else if nobs <= &q1 then x_quint=3;
else if nobs <= &q2 then x_quint=2;
else x_quint=1;
run;
...
proc anova data=test3;
class timeperiod;
model nobs = timeperiod;
by tx_agree;
means timeperiod;
run;

proc print data=test3(OBS=10); RUN;

/* Frequencies for tables */
proc freq data = test3;
table diag_agree*timeperiod;
table diag_agree*new_train;
table diag_agree*x_quint;
table diag_agree*new_educ;
table diag_agree*new_expgr;
table diag_agree*pt_sex;
run;


proc freq data = test3;
table tx_agree*timeperiod;
table tx_agree*new_train;
table tx_agree*x_quint;
table tx_agree*new_educ;
table tx_agree*new_expgr;
table tx_agree*pt_sex;
run;




/* random int for health center + nurse */
/* diagnosis crude*/
proc glimmix empirical data=test3 method=quad;
class pt_sex nurseid new_train timeperiod imai_train health_center;
model diag_agree = time /solution dist=binary oddsratio;
random int/subject=nurseid;
random int/subject=health_center(nurseid) type=vc;
run;

proc glimmix empirical data=test3 method=quad;
class nurseid new_train imai_train timeperiod health_center;
model diag_agree = new_train/solution dist=binary oddsratio;
random int/subject=nurseid;
random int/subject=health_center(nurseid) type=vc;
estimate 'oct vs. mar' new_train 1 -1 0 /exp cl;
run;

proc glimmix empirical data=test3 method=quad;
class nurseid new_educ health_center;
model diag_agree = new_educ /solution dist=binary oddsratio;
random int/subject=nurseid;
random int/subject=health_center(nurseid) type=vc;
run;

proc glimmix empirical data=test3  method=quad;
class nurseid  new_expgr health_center;
model diag_agree =  new_expgr/solution dist=binary oddsratio;
random int/subject=nurseid;
random int/subject=health_center(nurseid) type=vc;
run;
...
proc glimmix empirical data=test3  method=quad;
class nurseid  new_expgr health_center;
model diag_agree =  nobs/solution dist=binary oddsratio;
random int/subject=nurseid;
random int/subject=health_center(nurseid) type=vc;
run;

proc glimmix empirical data=test3  method=quad;
class nurseid x_quint new_expgr health_center;
model diag_agree =  x_quint/solution dist=binary oddsratio;
random int/subject=nurseid;
random int/subject=health_center(nurseid) type=vc;
run;
...
proc glimmix empirical data=test3 method=quad;
class pt_sex nurseid new_train timeperiod imai_train health_center;
model tx_agree = time /solution dist=binary oddsratio;
random int/subject=nurseid;
random int/subject=health_center(nurseid) type=vc;
run;
...
proc glimmix empirical data=test3 method=quad;
class nurseid new_train imai_train timeperiod health_center;
model tx_agree = new_train/solution dist=binary oddsratio;
random int/subject=nurseid;
random int/subject=health_center(nurseid) type=vc;
estimate 'oct vs. mar' new_train 1 -1 0 /exp cl;
run;
...
proc glimmix empirical data=test3 method=quad;
class nurseid new_educ health_center;
model tx_agree = new_educ /solution dist=binary oddsratio;
random int/subject=nurseid;
random int/subject=health_center(nurseid) type=vc;
run;
...
proc glimmix empirical data=test3  method=quad;
class nurseid  new_expgr health_center;
model tx_agree =  new_expgr/solution dist=binary oddsratio;
random int/subject=nurseid;
random int/subject=health_center(nurseid) type=vc;
run;
...
proc glimmix empirical data=test3  method=quad;
class nurseid  new_expgr health_center;
model tx_agree =  nobs/solution dist=binary oddsratio;
random int/subject=nurseid;
random int/subject=health_center(nurseid) type=vc;
run;
...
proc glimmix empirical data=test3  method=quad;
class nurseid x_quint new_expgr health_center;
model tx_agree =  x_quint/solution dist=binary oddsratio;
random int/subject=nurseid;
random int/subject=health_center(nurseid) type=vc;
run;
...
proc glimmix empirical data=test3 method=quad;
class pt_sex nurseid new_train timeperiod imai_train x_quint health_center;
model tx_agree = timeperiod new_train x_quint/solution dist=binary oddsratio;
random int /subject=nurseid;covtest/wald;
random int /subject=health_center(nurseid) type=vc;covtest/wald;
run;

proc glimmix empirical data=test3 method=quad;
class nurseid  timeperiod new_train x_quint health_center;
model diag_agree =  timeperiod new_train/solution dist=binary oddsratio;
random int /subject=nurseid;covtest/wald;
random int /subject=health_center(nurseid) type=vc;covtest/wald;
run;


/* Subset: Nurses in both Pre and Post period */

data test4;
set test3;
if (nurseID  in (201, 203, 204, 304, 305, 306, 401, 502, 503, 506, 601, 603, 704, 801, 802));
run;

proc glimmix empirical data=test4 method=quad;
class nurseid  timeperiod new_train x_quint health_center pt_sex new_expgr new_educ;
model tx_agree = timeperiod new_train x_quint pt_sex/solution dist=binary oddsratio;
random int /subject=nurseid solution;covtest/wald;
run;

proc glimmix empirical data=test4 method=quad;
class nurseid  timeperiod new_train x_quint health_center pt_sex new_expgr new_educ;
model diag_agree = timeperiod new_train new_educ/solution dist=binary oddsratio;
random int /subject=nurseid;
covtest/wald;
run;
...
proc glimmix empirical data = test4 method =quad;
class nurseid  timeperiod new_train x_quint health_center pt_sex new_expgr new_educ;;
model diag_agree = new_timeperiod new_train educ/solution dist=binary oddsratio;
random int /subject=nurseid;
covtest/wald;
run;
